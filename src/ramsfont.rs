extern crate libc;
use crate::ffi::*;
use libc::{c_float, c_int, c_short, c_uint};
use std::ffi::CString;
...
impl RAMSoundFont {
    ...
    pub fn set_name(&self, name: &str) -> i32 {
        ...
        unsafe { fluid_ramsfont_set_name(self.to_raw(), name_str.as_ptr()) }
    }
...
impl RAMSample {
    ...
    pub fn set_name(&self, name: &str) -> i32 {
        let name_str = CString::new(name).unwrap();
        unsafe { fluid_sample_set_name(self.to_raw(), name_str.as_ptr()) }
    }

    pub fn set_sound_data(
        &self,
        data: *mut i16,
        nbframes: u32,
        copy_data: bool,
